...
<body>
	...
	<div id="header">
		...
		<p><%=id%></p>
		<div id="workerType" name="check">检查员</div>
	</div>
	...
	<div id="content">
		...
		<div id="content_left">
			...
			<!-- 历史纪录 -->
			<a href="selecthistory.do" target="_blank"><input
				class="imageBtnHistory" id="imageBtnHistory" type="button"
		...
		<%
			if (picture != null) {
		%>
		<img id="image" src="<%=picture.getPadd()%>"
			onload="getSignFromServer()"></img>
		<%
			} else {
		%>
	...
	<form id="formTest" method="post" action="addtagcheck.do"
		style="display: none;">
		<input type="checkbox" name="xl" value='0' checked></input> <input
			...
			type="checkbox" name="yr" value='0' checked></input><input
			type="checkbox" name="delteList" value='0' checked></input>
		<%
			if (picture != null) {
		%>
